...
import { register } from "../hooks/auth.js";
...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateName(data?.userName)) {
      ...
      setError(
        " *Password should start with capital letter and maxm length 7 "
      );
      setIsError(true);
    } else if (
      data?.password === " " ||
  ...
  return (
    <div className=" bg-blue-950 max-h-fit py-10 xl:pt-24   px-10 xl:px-96">
      <form
        className=" bg-slate-500 bg-opacity-50 text-gray-400 md:w-[50vx] md:mx-32 py-10 px-5 xl:px-20 rounded-lg "
        key={formkey}
      >
        ...
        <div className=" w-full py-4 px-6 md:px-8 grid grid-flow-row lg:flex justify-between">
          <input
            ...
            className="mx-0 w-[42vw] lg:w-[70%] h-8 px-2 text-black"
            onChange={(e) => handleDataChange("avatar", e)}
            placeholder="Avatar"
          />
        </div>
        {isError && <div className=" text-red-700 px-10 "> {error}</div>}
